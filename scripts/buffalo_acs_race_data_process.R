...
file_directories <- list.files(here("data/acs/race/raw"), full.names = TRUE)

#storing all files into a list===
NY_acs_race_data <- lapply(file_directories, function(x) read_csv(x, show_col_types = FALSE))

# Data Validation====
#total count of all datasets==
file_count <- length(NY_acs_race_data)

#grabbing readable file names==
file_names <- list.files(here("data/acs/race/raw"))

#total count of all variables in each dataset==
var_counts <- sapply(NY_acs_race_data, function(x) length(names(x)), simplify = TRUE)


#pulling all present variables names==
var_names <- unique(unlist(lapply(NY_acs_race_data, function(x) unique(names(x)))))
...
if(sum(var_names == expected_cols) != length(expected_cols)){
  full_names <- lapply(NY_acs_race_data, function(x) unique(names(x)))
