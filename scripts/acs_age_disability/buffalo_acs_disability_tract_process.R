#==== Buffalo Census Tracts - ACS Disability Data Processing Script====#
...
acsvariables_list <- map(year, ~load_variables(.x, "acs5", cache = TRUE) %>%
                           filter(str_detect(name, disability_code)))
...
expected_vars <- `names<-`(readRDS("scripts/utilities/acs_disability_expected.RDS"), NULL)
...
# Prepping for pulling the right variables
acs_vars <-`names<-`(acs_variables$name, acs_variables$label)
