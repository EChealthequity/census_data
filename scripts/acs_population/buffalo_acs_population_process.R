#==== Buffalo - ACS Total Population Data Processing Script====#
...
# Pulling the "Total Population" variable helper==
...
ALL_NY_place_total <- map(years, ~ get_acs(geography = "place",
                                          state = "NY",
                                          year = .x,
                                          survey = "acs1",
...
# Affixing names==
names(ALL_NY_place_total) <- years

# Collapsing all buffalo results into a single data frame====
buffalo_acs_total_10_19 <- bind_rows(ALL_NY_place_total, .id = "year")

# Saving the data to the directory====
write_csv(buffalo_acs_total_10_19, "data/acs/population/Buffalo ACS Total Population Data - 2010-2019.csv")
...
cloud_saver("Buffalo ACS Total Population Data 2010 to 2019", buffalo_acs_total_10_19)
