#==== New York State - ACS Total Data Processing Script====#
...
# Pulling all race data for New York====
# Only pulling variables we need
ALL_NYS_total <- map(years, ~ get_acs(geography = "state",
                                     state = "NY",
                                     year = .x,
                                     survey = "acs1",
...
# Affixing names==
names(ALL_NYS_total) <- years

# Collapsing all NYS results into a single data frame====
nys_acs_total_10_19 <- bind_rows(ALL_NYS_total, .id = "year")

# Saving the data to the directory====
write_csv(nys_acs_total_10_19, "data/acs/population/NYS ACS Total Population Data - 2010-2019.csv")
...
cloud_saver("NYS ACS Total Population Data 2010 to 2019", nys_acs_total_10_19)
